/**
 * microBees Javascript SDK
 * @company microBees Technology LTD
 * @copyright (C) 2015 microBees Technology LTD
 */

var hello = function(name){
	return hello.use(name);
};
var server="https://dev.microbees.com";
hello.utils = {
	//
...
hello.init({
	microbees : {
		...
		login : function(p){
			...
			p.options.window_width = 800;
			p.options.window_height = 600;
		},

		// https://developers.facebook.com/docs/facebook-login/manually-build-a-login-flow/v2.1
		oauth : {
			version : 2,
			auth : server+'/beessmart-core/oauth/authorize',
			grant : server+'/beessmart-core/oauth/token'
		},
...
function microBees(clientID){
	this.debug= false;
	this.token="";
	this.logged = false;
	this.login = function(callback){
		if(this.debug)
			console.log("logging with client "+clientID);
		hello.init({ microbees : clientID},{redirect_uri:window.location.href});
				hello( "microbees" ).login().then( function(){
					var response = hello( "microbees" ).getAuthResponse();
	...
	this.doRequest = function(url,params,callback){ 	
		var req;
